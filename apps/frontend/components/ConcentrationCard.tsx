  ...
  return (
    <section className="rounded-2xl border border-divider bg-surface-elevated p-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="mono text-xs text-on-surface">
      ...
      <div className="space-y-1.5">
        {stats.bars.map((b) => (
          <div key={b.outcomeId} className="flex items-center gap-2 text-[11px]">
            <span className="w-28 truncate text-on-surface-muted">{b.name}</span>
            ...
            <span className="mono w-10 text-right text-on-surface">
              {b.pct.toFixed(0)}%
            </span>
          </div>
        ))}
      ...
      <div className="mt-3 text-[10px] text-on-surface-muted">
        {stats.note}
      </div>
    </section>
  );
